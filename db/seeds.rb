...
5.times do |i|
  ...
  property_apartment_for_sell.image.attach(
    io: File.open(Rails.root.join "db/images/#{image_file_name}"),
    filename: image_file_name
  )
  property_apartment_for_rent.image.attach(
    io: File.open(Rails.root.join "db/images/#{image_file_name}"),
    filename: image_file_name
  )
  property_villa_for_sell.image.attach(
    io: File.open(Rails.root.join "db/images/#{image_file_name}"),
    filename: image_file_name
  )
  property_villa_for_rent.image.attach(
    io: File.open(Rails.root.join "db/images/#{image_file_name}"),
    filename: image_file_name
  )
